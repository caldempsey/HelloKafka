...
url=$(curl --stderr /dev/null "https://www.apache.org/dyn/closer.cgi?path=/kafka/kafka_2.1.1-2.1.0.tgz&as_json=1" | jq -r '"\(.preferred)\(.path_info)"')
...
echo "Downloading Kafka tar from $url and saving to /tmp/kafka.tgz"
